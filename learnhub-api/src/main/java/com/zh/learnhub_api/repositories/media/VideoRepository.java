...
public interface VideoRepository extends JpaRepository<Video, Long> {
    ...
    @Query("SELECT c.id AS courseId, v.storageKey AS storageKey, v.status AS status, "
         ...
         + "FROM Video v JOIN v.lessonId l JOIN l.courseId c WHERE v.id = :videoId")
    Optional<VideoPlaybackProjection> findPlaybackById(@Param("videoId") Long videoId);

    @Query("SELECT c.id AS courseId, v.storageKey AS storageKey, v.status AS status, "
         + "l.isPreview AS lessonPreview, c.status AS courseStatus "
         + "FROM Video v JOIN v.lessonId l JOIN l.courseId c "
    ...
    Optional<Video> findByLessonIdAndPosition(Lesson lesson, Integer position);
    ...
    List<Video> findByLessonId_IdOrderByPositionAsc(Long lessonId);

    @Query("SELECT v FROM Video v WHERE v.lessonId.courseId.id = :courseId "
         + "ORDER BY v.lessonId.position ASC, v.position ASC, v.id ASC")
    List<Video> findAllByCourseIdOrdered(@Param("courseId") Long courseId);
    ...
    @Query("SELECT v FROM Video v WHERE v.lessonId.courseId.id = :courseId "
         ...
         + "ORDER BY v.lessonId.position ASC, v.position ASC")
    ...
    @Query("SELECT v.mediaconvertJobId FROM Video v "
         + "WHERE v.lessonId.courseId.id = :courseId "
         + "AND v.status = :status "
