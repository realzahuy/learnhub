...
import com.zh.learnhub_api.projections.account.UserRoleProjection;
...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    List<ExistingUserProjection> findExisting(
    ...
    @Query(value = "SELECT u.id AS id, u.email_verified AS emailVerified, " +
    ...
    @Query("SELECT u.id AS userId, r.name AS roleName "
         + "FROM User u JOIN u.roleSet r WHERE u.id IN :userIds")
    ...
    @Query(value = "SELECT CASE "
           ...
           nativeQuery = true)
    List<TimeBucketCountProjection> countCreatedByBucket(@Param("from") LocalDateTime from,
    ...
    @Query(value = "SELECT CASE "
                 ...
                 + "GROUP BY bucket ORDER BY bucket",
           nativeQuery = true)
    List<TimeBucketCountProjection> countCreatedByBucketWithRole(@Param("from") LocalDateTime from,
                                                                 @Param("to") LocalDateTime to,
                                                                 @Param("groupBy") String groupBy,
                                                                 @Param("roleName") String roleName);
