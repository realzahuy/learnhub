...
public interface EnrollmentRepository extends JpaRepository<Enrollment, Long> {
    ...
    @Query("SELECT e.courseId.id FROM Enrollment e " + "WHERE e.userId.id = :userId AND e.courseId.id IN :courseIds")
    Set<Long> findCourseIdsByUserIdAndCourseIds(@Param("userId") Long userId, @Param("courseIds") List<Long> courseIds);
    ...
    @Query(
            value = "SELECT e.id AS enrollmentId, c.id AS courseId, "
    ...
    @Query(
            ...
            nativeQuery = true)
    List<TimeBucketCountProjection> countEnrollmentsByBucket(
    ...
    @Query(
            value = "SELECT CASE "
                    ...
                    + "GROUP BY bucket ORDER BY bucket",
            nativeQuery = true)
    List<TimeBucketCountProjection> countNewStudentsByBucket(
            @Param("instructorId") Long instructorId,
            @Param("from") LocalDateTime from,
            @Param("to") LocalDateTime to,
            @Param("groupBy") String groupBy);
    ...
    interface InstructorStudentCountProjection {
