...
public class ChatAssistantService {
    ...
    public ChatResponseDTO respond(ChatRequestDTO request, Long userId) {
        ...
        List<RecommendationCardDTO> courses;
        try {
            courses = plan.shouldSearchCourses()
                    ? recommendationService.recommend(plan.searchKeywords(), userId)
                    : List.of();
        } catch (RestClientException ex) {
            log.warn("Chat recommendations unavailable", ex);
            return new ChatResponseDTO(plan.reply(), List.of());
        }
